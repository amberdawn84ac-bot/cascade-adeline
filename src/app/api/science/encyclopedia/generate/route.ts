...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await llm.invoke([
      {
        role: 'system',
        content: `You are Adeline, generating an INTERACTIVE SCIENCE LESSON for a specific student. 

FORBIDDEN — DO NOT OUTPUT:
- Lab report templates
- Hypothesis/Observation/Conclusion formats
- "Write down what you see" instructions
- Passive observation exercises
- Wikipedia-style articles

REQUIRED — OUTPUT AN INTERACTIVE LESSON:
...
You MUST tailor every word to the student's exact grade level, reading level, vocabulary, and cognitive profile provided below. This is non-negotiable.
- A 1st grader gets wonder, simple language, concrete sensory details ("squishy," "smells like dirt")
